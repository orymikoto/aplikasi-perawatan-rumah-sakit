...
@section('content')
  <div class="flex flex-col justify-start items-start gap-6">
    ...
    <form action="{{ route('pasiens.update', $pasien_dirawat->id) }}" class="flex flex-col gap-2 p-4 rounded-2xl shadow-md xl:w-[480px] " method="POST">
      ...
      <x-input-text placeholder="Jenis Penyakit..." name="jenis_penyakit" title="Jenis Penyakit" value="{{ $pasien_dirawat->jenis_penyakit }}" />
      ...
      <x-input.date title="Tanggal Masuk" name="tanggal_masuk" placeholder="Pilih tanggal masuk" value="{{ $pasien_dirawat->tanggal_masuk }}" />
