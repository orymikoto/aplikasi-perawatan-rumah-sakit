...
@section('content')
    <div class="flex flex-col justify-start items-start gap-6">
        ...
        <form action="{{ route('pasiens.update', $pasien_dirawat->id) }}"
            class="flex flex-col gap-2 p-4 rounded-2xl shadow-md xl:w-[480px] " method="POST">
            @method('PUT')
            @csrf
            <x-input.text title="No. RM" name="no_rm" placeholder="Nomor RM..."
                value="{{ $pasien_dirawat->pasien->no_RM }}" />
            <x-input.text title="Nama Pasien" name="nama_pasien" placeholder="Nama Pasien ..."
                value="{{ $pasien_dirawat->pasien->nama }}" />
            <x-input.number title="Usia" name="umur" placeholder="Usia Pasien..."
                value="{{ $pasien_dirawat->pasien->umur }}" />
            <x-input.select title="Jenis Kelamin" name="jenis_kelamin" placeholder="Pilih jenis kelamin pasien"
                :options="['LAKI - LAKI', 'PEREMPUAN']" value="{{ $pasien_dirawat->pasien->jenis_kelamin }}" />
            <x-input.date title="Tanggal Daftar Pasien" name="tanggal_daftar" placeholder="Tanggal Daftar Pasien..."
            ...
            <x-input.text placeholder="Kode Penyakit..." name="kode_penyakit" title="Kode Penyakit"
                value="{{ $pasien_dirawat->kode_penyakit }}" />
            <x-input.text placeholder="Jenis Penyakit..." name="jenis_penyakit" title="Jenis Penyakit"
                value="{{ $pasien_dirawat->jenis_penyakit }}" />
            ...
            <x-input.date title="Tanggal Masuk" name="tanggal_masuk" placeholder="Pilih tanggal masuk"
                value="{{ $pasien_dirawat->tanggal_masuk }}" />
