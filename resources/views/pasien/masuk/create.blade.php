        ...
        <div class="form-group flex flex-col font-jakarta-sans gap-2">
          ...
          <datalist id="kode_penyakit_fieldlist">
            @foreach ($daftar_penyakit as $key => $value)
              <option value="{{ $value->kode_penyakit }}">{{ $value->kode_penyakit . ' - ' . $value->nama_penyakit }}</option>
            @endforeach
          </datalist>
    ...
    cek_rm.onclick = function() {
      ...
      fetch(`/pasiens/check-rm/${nomor_rm}`, {
        ...
        if (!res.error) {
          ...
          let jenis_kelamin = document.getElementsByName('jenis_kelamin')[0].value = res.jenis_kelamin;
          let tanggal_daftar = document.getElementsByName('tanggal_daftar')[0].value = res.tanggal_daftar.substring(0, 10);
          ...
          document.getElementById("pesan_no_rm").innerText = `Data pasien atas nama ${res.nama} ditemukan!`;
          document.getElementById("pesan_no_rm").classList = "text-emerald-600";
          document.getElementById("submit_button").removeAttribute("disabled")

        } else {
          document.getElementById("pesan_no_rm").innerText = "Data pasien tidak ditemukan, silahkan tambah pasien baru!"
          document.getElementById("pesan_no_rm").classList = "text-red-600";
