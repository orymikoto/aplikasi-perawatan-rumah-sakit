    ...
    <table class="rounded-t-2xl w-full min-h-full flex-1 shadow-md overflow-hidden table ">
      <thead>
        <tr class="text-white font-josefin-sans font-medium h-16 text-lg bg-emerald-600">
          ...
          <th class="px-4 text-center w-52">Jenis Pembayaran</th>
          <th class="px-4 text-center w-32">Tgl Masuk</th>
          <th class="px-4 text-center w-16">Aksi</th>
        </tr>
      </thead>
      <tbody>
        @forelse ($pasien_dirawats as $pasien)
          <tr data-entry-id="{{ $pasien->id }}" class="h-12 text-lg font-josefin-sans font-medium text-neutral-800">
            ...
            <td class="px-4">{{ $pasien->tanggal_masuk }}</td>
            <td class="px-4">
              <div class="flex gap-2 mx-auto justify-center">
                ...
                <a href="{{ route('pasiens.edit', $pasien->id) }}" class="bg-teal-600 text-white hover:bg-teal-500 p-1 px-2 rounded-md">
