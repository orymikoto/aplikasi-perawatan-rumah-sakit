      ...
      <table class="rounded-t-2xl w-full min-h-full flex-1 shadow-md overflow-hidden table ">
        <thead>
          <tr class="text-white font-josefin-sans font-medium h-16 text-lg bg-emerald-600">
            ...
            <th class="px-4 text-center min-w-56">Dirujuk Ke</th>
            <th class="px-4 text-center min-w-56">Jam Dirujuk</th>
          ...
          @forelse ($pasien_keluar as $pasien)
            <tr data-entry-id="{{ $pasien->id }}"
              ...
              <td class="text-center">
                {{ ($pasien_keluar->currentPage() - 1) * $pasien_keluar->links()->paginator->perPage() + $loop->iteration }}
              <td class="px-4">{{ $pasien->pasien->no_RM }}</td>
              <td class="px-4">{{ $pasien->pasien->nama }}</td>
              <td class="px-4 text-start">{{ $pasien->kode_penyakit }}</td>
              <td class="px-4 text-start">{{ $pasien->penyakit->nama_penyakit }}</td>
              ...
              <td class="px-4 text-center">{{ $pasien->jam_dirujuk ?? '-' }}</td>
              {{-- <td class="px-4">
