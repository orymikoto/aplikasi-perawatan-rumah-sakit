...
<div class="relative">
  <button onclick="showModal(event, '{{ $id }}')" class="px-2 py-2 flex items-center justify-center bg-blue-500 text-white rounded">
    <i class="fa-solid fa-ellipsis"></i>
  </button>

  <div id="{{ $id }}" class="w-full h-full z-10 fixed bg-neutral-900/10 hidden top-0 left-0" onclick="hideModal('{{ $id }}')">
    <div id="popup.{{ $id }}" class="fixed rounded-lg shadow-lg flex flex-col gap-2 overflow-hidden bg-white z-20">
...
<div id="{{ 'pasien_keluar_' . $id_pasien }}" class="fixed z-10 top-0 left-0 bg-slate-900/10 hidden justify-center items-center w-screen h-screen">
    ...
    <form action="{{ route('pasien_keluar', $id_pasien) }}" class="flex flex-col gap-2" method="POST">
      ...
      <div class="flex flex-col gap-2 font-jakarta-sans">
        ...
        <select onchange="checkSelectedPasienKeluar('{{ $id_pasien }}')"
          class="w-full border border-neutral-600 focus:border-emerald-600 font-medium rounded-lg py-2 px-3 outline-none" name="kondisi"
          id="{{ 'kondisi_' . $id_pasien }}" required>
      ...
      <x-input.text title="Rumah Sakit Rujukan" id="{{ 'rumah_sakit_' . $id_pasien }}" :required="false" name="rumah_sakit"
        placeholder="Nama rumah sakit ..." value="" class="hidden" />
      <x-input.text type="time" title="Jam Rujukan" id="{{ 'jam_dirujuk_' . $id_pasien }}" :required="false" name="jam_rujuk"
        placeholder="Nama rumah sakit ..." value="" class="hidden" />
      <div class="flex justify-stretch w-full gap-x-2">
        <x-button.color-button onclick="hidePasienModal('pasien_keluar_{{ $id_pasien }}')" class="flex-1" text="Batal" color="red-600"
          type="reset" />
...
<div id="{{ 'pasien_pindah_' . $id_pasien }}" class="fixed z-10 top-0 left-0 bg-slate-900/10 hidden justify-center items-center w-screen h-screen">
  <div class="p-6 flex flex-col gap-2 rounded-lg bg-white">
    <h3>Pasien Pindah</h3>
    <form action="{{ route('pasien_pindah', $id_pasien) }}" class="flex flex-col gap-2" method="POST">
      @csrf
      <x-input.select title="Pindah Ruang" id="{{ 'ruangan_' . $id_pasien }}" name="ruangan" placeholder="Pilih Ruangan Baru..." value=""
        :options="$daftar_ruangan" />
      <div class="flex justify-stretch w-full gap-x-2">
        <x-button.color-button onclick="hidePasienModal('pasien_pindah_{{ $id_pasien }}')" class="flex-1" text="Batal" color="red-600"
          type="reset" />
        <x-button.color-button text="Simpan" type="submit" class="flex-1" />
      </div>
    </form>
...
<script>
  ...
  function checkSelectedPasienKeluar(id) {
    ...
    if (select_pasien == 'Keluar - Dirujuk') {
      ...
      var rumah_sakit_input = document.getElementById(`jam_dirujuk_${id}`).classList.remove('hidden')
    } else {
      var rumah_sakit_input = document.getElementById(`rumah_sakit_${id}`).classList.add('hidden')
      var rumah_sakit_input = document.getElementById(`jam_dirujuk_${id}`).classList.add('hidden')
    }
  }
